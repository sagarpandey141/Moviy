...
const DetailPage = () => {
    ...
    const{flag}=useSelector((state)=>state.DetailFlag);
    ...
    async function GetSpecificMovieData()
  {
    let response=0;
     if(flag==false){ 
      response= await apiConnector("GET",movieUrls.MOVIE_DETAIL+`${movieId}`);
     } else{
      response= await apiConnector("GET",movieUrls.Tv_Detail+`${movieId}`);
     }
  ...
  async function GetCredit(){
            ...
            let response=0;
            if(flag==false){ 
              response= await apiConnector("GET",movieUrls.MOVIE_DETAIL+`${movieId}/credits`);
            } else{
              response= await apiConnector("GET",movieUrls.Tv_Detail+`${movieId}/credits`);
            }
            if(response){
  ...
  async function getOfficialVideo(){
       ...
       let response=0;
        if(flag==false){ 
          response= await apiConnector("GET",movieUrls.MOVIE_DETAIL+`${movieId}/videos`);
        } else{
          
          response= await apiConnector("GET",`https://api.themoviedb.org/3/tv/${movieId}/videos`);
        }
  ...
  async function getSimilarVideo(){
         ...
         let response=0;
        if(flag==false){ 
          response= await apiConnector("GET",movieUrls.MOVIE_DETAIL+`${movieId}/similar`);
        } else{
          response= await apiConnector("GET",movieUrls.Tv_Detail+`${movieId}/similar`);
        }
  ...
  async function RecommendadVideo(){
        ...
        let response=0;
        if(flag==false){ 
          response= await apiConnector("GET",movieUrls.MOVIE_DETAIL+`${movieId}/recommendations`);
        } else{
          response= await apiConnector("GET",movieUrls.Tv_Detail+`${movieId}/recommendations`);
        }
        ...
        SpecificData.length===0 ? (
          ...
          <div className='flex  justify-center  gap-20 bg-[#08172f] '>
              ...
              <div className=' mt-28 flex  justify-center  gap-20 text-white '>
                      {/*left part*/}
                    <div>
                          <img className=' h-[32rem] rounded-md' src={imageBaseUrl+"w400"+SpecificData?.data?.poster_path}/>
                    </div>
                
                {/*right part*/}
                <div className='max-w-[50%]'>
                          <h2 className=' text-4xl'>{SpecificData?.data?.belongs_to_collection?.name}</h2>
                          <p className=" text-slate-400 italic mt-1 text-2xl">{SpecificData?.data?.tagline}</p>

                        {/*genres*/}
                          <div className='flex gap-3 mt-1 '>
                              {
                                SpecificData?.data?.genres?.map((data,index)=>{
                    ...
                    <CircularProgressbar
                        maxValue={10} value={SpecificData?.data?.vote_average} text={`${SpecificData?.data?.vote_average?.toFixed(2)}%`}
                        background={true}
                        backgroundPadding={5}

                        styles={buildStyles({
                          pathColor: SpecificData?.data?.vote_average >= 7 ? "green" : "#FFA41B",
                    ...
                    <div>
                        ...
                        <p className='text-lg mt-2'>{SpecificData?.data?.overview}</p>
                    </div>
                    {/*status released runtime */}
                    <div className='flex gap-2 mt-7'>
                        <p className=' text-lg'>Status: <span className=' text-slate-500'>{SpecificData?.data?.status}</span></p>
                        <p className='text-lg'>Release Date: <span className=' text-slate-500'>{SpecificData?.data?.release_date}</span></p>
                        <p className='text-lg'>Runtime: <span className=' text-slate-500'>{SpecificData?.data?.runtime}</span></p>
                    </div>
