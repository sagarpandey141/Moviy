...
const Testing = () => {
  ...
  async function GetDirector() {
    const response = await apiConnector(
      "GET",
      movieUrls +
        `movie/${movieId}/credits?api_key=AIzaSyBf-zXFP5QpHzoyX6DndKdir2VCNDYmaWI`
    );
  }
    ...
    <div className="bg-[#08172f] w-screen h-max  overflow-hidden">
      <div className="relative max-w-4xl w-11/12 mx-auto">
          ...
          <div className="mt-28 flex  justify-center gap-20 text-white">
            ...
            <div>
              <img
                ...
                src={
                  imageBaseUrl + "w400" + MovieSpecificData?.data?.poster_path
                }
              />
            </div>
            ...
            <div className="max-w-[50%]">
              ...
              <p className=" text-slate-400 italic mt-1 text-2xl">
              ...
              <div className="flex gap-3 mt-1 ">
                {MovieSpecificData?.data?.genres?.map((data, index) => {
                  return (
                    <p key={index} className=" bg-pink-600 p-1 rounded-md">
                      {data?.name}
                    </p>
                  );
                })}
              ...
              <div className=" flex mt-4  rounded-full bg-slate-800 w-24 h-24 items-center text-white">
                <CircularProgressbar
                  maxValue={10}
                  value={MovieSpecificData?.data?.vote_average}
                  text={`${MovieSpecificData?.data?.vote_average?.toFixed(2)}%`}
                  background={true}
                  backgroundPadding={5}
                  styles={buildStyles({
                    pathColor:
                      MovieSpecificData?.data?.vote_average >= 7
                        ? "green"
                        : "#FFA41B",
        ...
        {SimilarVideo.length > 0 && (
          <div className=" my-6">
            <h2 className="text-white text-3xl">Similar Video</h2>
        ...
        {Recommended.length > 0 && (
          <div className=" my-6">
            <h2 className="text-white text-2xl ">Recommendations</h2>
      ...
      {modal && <Modal data={modal} setModal={setModal}></Modal>}
    </div>
  );
};
